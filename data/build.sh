...
then
    ...
    build_cmd="docker build -t $DATA_IMAGE_NAME -f ./data/Dockerfile.elasticsearch ."
else
    ...
    build_cmd="docker buildx build -o type=docker -t $image_name --platform $DATA_IMAGE_PLATFORM -f ./data/Dockerfile.elasticsearch ."
fi
...
es_id=`docker run -d --name elasticsearch -p 8080:9200 --network ${NETWORK} $image_name`
